...
# Get cpu count
case $LP_OS in
    Linux)   _lp_CPUNUM=$( nproc 2devnull  grep -c '^[Pp]rocessor' proccpuinfo ) ;;
    FreeBSDDarwin) _lp_CPUNUM=$( sysctl -n hw.ncpu ) ;;
...
#    for configtheme. Those variables are local to the function.
...
_lp_source_config()
{
    ...
    local ti_sgr0=$( { tput sgr0  tput me ; } 2devnull )
    local ti_bold=$( { tput bold  tput md ; } 2devnull )
    local ti_setaf
    if tput setaf devnull 2&1 ; then
        ti_setaf () { tput setaf $1 ; }
    elif tput AF devnull 2&1 ; then
        # BSD
        ti_setaf () { tput AF $1 ; }
    else
    ...
    LP_SCREEN_TITLE_CLOSE=${LP_SCREEN_TITLE_CLOSE-033134}
    ...
    # Default config file may be the XDG standard ~.configliquidpromptrc,
    # but heirloom dotfile has priority.

    local configfile
    if [[ -f etcliquidpromptrc ]]
    then
        source etcliquidpromptrc
    fi
    ...
    then
        configfile=$HOME.configliquidpromptrc
    else
        configfile=$XDG_HOME_DIRliquidpromptrc
    fi
    if [[ -f $configfile ]]
    then
        source $configfile
...
[[ $LP_ENABLE_GIT  = 1 ]] && { command -v git  devnull  LP_ENABLE_GIT=0  ; }
[[ $LP_ENABLE_SVN  = 1 ]] && { command -v svn  devnull  LP_ENABLE_SVN=0  ; }
[[ $LP_ENABLE_FOSSIL  = 1 ]] && { command -v fossil  devnull  LP_ENABLE_FOSSIL=0  ; }
[[ $LP_ENABLE_HG   = 1 ]] && { command -v hg   devnull  LP_ENABLE_HG=0   ; }
[[ $LP_ENABLE_BZR  = 1 ]] && { command -v bzr  devnull  LP_ENABLE_BZR=0  ; }
[[ $LP_ENABLE_BATT = 1 ]] && { command -v acpi devnull  LP_ENABLE_BATT=0 ; }
...
_chroot()
{
    if [[ -r etcdebian_chroot ]] ; then
        local debchroot
        debchroot=$(cat etcdebian_chroot)
        echo (${debchroot})
    fi
}
LP_HOST=$(_chroot)
...
# BASHZSH function that shortens
...
# httphbfs.wordpress.com20090901short-pwd-in-bash-prompts
#
...
_lp_shorten_path()
{
    ...
    local max_len=$((${COLUMNS-80}$LP_PATH_LENGTH100))
    local mask_len=${#mask}
    local slashes=0
...
_lp_get_dirtrim() {
    ...
    if [[ $((len)) -gt $((max_len)) ]]; then
        ...
        do
            ...
            [[ ${pi1} ==  ]] && PROMPT_DIRTRIM=$((PROMPT_DIRTRIM+1))
        done
...
# - detached screens sessions andor tmux sessions running on the host
...
_lp_jobcount_color()
{
    ...
    local n_screen=$(screen -ls 2 devnull  grep -c Detach)
    local n_tmux=$(tmux list-sessions 2 devnull  grep -cv attached)
...
_lp_git_branch()
{
    ...
    local gitdir
    gitdir=$(git rev-parse --git-dir 2devnull)
    [[ $ -ne 0  ! $gitdir =~ (.).git. ]] && return
    local branch=$(git symbolic-ref HEAD 2devnull)
    if [[ $ -ne 0  -z $branch ]] ; then
        # In detached head state, use commit instead
        branch=$(git rev-parse --short HEAD 2devnull)
    fi
    [[ $ -ne 0  -z $branch ]] && return
    branch=${branch#refsheads}
...
_lp_git_branch_color()
{
    ...
    if [[ ! -z $branch ]] ; then
        ...
        local GD
        git diff --quiet devnull 2&1
        GD=$

        local GDC
        git diff --cached --quiet devnull 2&1
        GDC=$

        local has_untracked
        has_untracked=$(git status 2devnull  grep '(# Untracked)')
        ...
        local has_stash
        has_stash=$(git stash list 2devnull)
        if [[ -z $has_stash ]] ; then
            has_stash=
        else
        ...
        local remote
        remote=$(git config --get branch.${branch}.remote 2devnull)
        # if git has no upstream, use origin
        if [[ -z $remote ]]; then
            remote=origin
        fi
        local remote_branch
        remote_branch=$(git config --get branch.${branch}.merge 2devnull)
        ...
        if [[ -n $remote && -n $remote_branch ]] ; then
            has_commit=$(git rev-list --no-merges --count $remote${remote_branch}..${branch} 2devnull)
        ...
        if [[ $GD -eq 1  $GDC -eq 1 ]] ; then
            local has_line
            has_lines=$(git diff --numstat 2devnull  awk 'NF==3 {plus+=$1; minus+=$2} END {printf(+%d-%dn, plus, minus)}')
...
_lp_hg_branch_color()
{
    ...
    if [[ ! -z $branch ]] ; then
        ...
        local has_untracked
        has_untracked=$(hg status 2devnull  grep '(^)'  wc -l)
        if [[ -z $has_untracked ]] ; then
            has_untracked=
        else
            has_untracked=$LP_COLOR_CHANGES$LP_MARK_UNTRACKED
        ...
        local has_commit
        has_commit=$(hg outgoing --no-merges ${branch} 2devnull  grep '(^changeset)'  wc -l)
        if [[ -z $has_commit ]] ; then
            has_commit=0
        fi
        ...
        if [[ $(( $(hg status --quiet -n  wc -l) )) = 0 ]] ; then
            ...
            local has_line
            has_lines=$(hg diff --stat 2devnull  tail -n 1  awk 'FS=  {printf(+%s-%sn, $4, $6)}')
            if [[ $has_commit -gt 0 ]] ; then
                # Changes to commit and commits to push
...
_lp_svn_branch()
{
    ...
    local url
    local result
    eval $(LANG=C LC_ALL=C svn info 2devnull  sed -n 's^URL (.)url=1p;s^Repository Root (.)root=1p' )
    if [[ $root ==  ]]; then
        return
    fi
...
_lp_fossil_branch()
{
    ...
    local branch
    branch=$(fossil status 2devnull  grep tags  cut -c17-)
    if [[ -n $branch ]] ; then
        echo $(_lp_escape $branch)
    else
        if fossil info &devnull ; then
            echo no-tag
        fi
    fi
}
...
_lp_bzr_branch_color()
{
    [[ $LP_ENABLE_BZR != 1 ]] && return
    local output
    output=$(bzr version-info --check-clean --custom --template='{branch_nick} {revno} {clean}' 2 devnull)
    local tuple=($output)
...
# Compute a gradient of backgroundforeground colors depending on the battery status
# Display
...
# Compute a gradient of backgroundforground colors depending on the battery status
_lp_load_color()
{
    ...
    local load
    local IFS= tn
    load=$(_lp_cpu_load  sed 's.g;s^0g' )
...
_lp_as_text()
{
    ...
    local pst=$(echo $1  sed -r sx1B[([0-9]{1,2}(;[0-9]{1,2}))[mK]g)
...
_lp_time_analog()
{
    ...
    #       utf   hex   int
    # hours 505B 909b 144155
    # half  5C67 9ca7 156167

    # The characters being grouped bas plainthirty, we must first now
...
_lp_set_prompt()
{
    ...
    case $LP_OS in
        LinuxFreeBSDSunOS) $LP_OLD_PROMPT_COMMAND ;;
        Darwin)
            case $(LP_DWIN_KERNEL_REL_VER) in
                1112) update_terminal_cwd ;;
    ...
    if [[ $LP_OLD_PWD != $PWD ]]; then
        ...
        if [[ $(_lp_are_vcs_disabled) -eq 0 ]] ; then
            ...
            if [[ -n $LP_VCS ]]; then
                ...
                if [[ -d $(git rev-parse --git-dir 2devnull)svn ]]; then
